...
read -p "Введите статический IP от провайдера (например, 10.10.10.10): " STATIC_IP
read -p "Введите маску подсети от провайдера (например, 24): " MASK
read -p "Введите шлюз от провайдера (например, 10.10.10.1): " GATEWAY
